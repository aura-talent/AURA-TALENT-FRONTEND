...
import { employerApi, type EmployerJob, type CandidateRow } from "@/lib/employerApi";
...
export default function EmployerTemplatesHub() {
  const { user } = useAuth();
  ...
  const [selectedCandidateUserId, setSelectedCandidateUserId] = useState<string>("");
  ...
  // 1. Load templates, employer context, favorites, popularity
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const items = await api.listTemplates("employer");
        setTemplates(items);
  ...
  useEffect(() => {
    if (selectedTemplate) {
      ...
      const hiringManagerPlaceholder = selectedTemplate.placeholders.find(p =>
        p.toLowerCase().includes("manager") || p.toLowerCase() === "hiring manager"
      );
      if (hiringManagerPlaceholder && user) {
        initVals[hiringManagerPlaceholder] = user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split("@")[0] || "";
      }
  ...
  const handleAutoPopulate = async () => {
    ...
    try {
      ...
      const res = await api.populatePlaceholders({
        ...
        role: "employer",
        candidate_user_id: selectedCandidateUserId || undefined,
      ...
      alert("Failed to auto-populate. Please verify you selected a valid Candidate and Job.");
    } finally {
  ...
  const handleDownloadZIP = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `aura_employer_templates_${Date.now()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  return (
    <div className="employer-page" style={{ padding: "1.5rem 1.5rem 6rem", minHeight: "90vh" }}>
      {/* Title Header */}
      <div className="employer-page-head" style={{ marginBottom: "1.5rem" }}>
        <div>
          <p className="eyebrow">EMPLOYER WORKSPACE</p>
          <h1>Candidate Communication Templates</h1>
          <p className="text-gray-500">Draft context-aware interview invites, rescheduling emails, and candidate rejections with Aura AI.</p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {savedList.length === 0 ? (
            <p style={{ fontSize: "0.8rem", color: "var(--ink-72)" }}>Your list is empty. Add templates from the library to prep several candidate messages and download them together.</p>
          ) : (
      ...
      {selectedTemplate && (
        ...
        >
          <div
            ...
            className="panel"
            style={{
              width: "min(920px, 100%)", maxHeight: "90vh", overflowY: "auto",
          ...
          >
            ...
            <div style={{ borderBottom: "1px solid var(--ink-06)", paddingBottom: "1rem", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <span className="eyebrow" style={{ color: "var(--iris)" }}>EDITOR WORKSPACE</span>
            ...
            {/* Context Picker */}
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", background: "var(--ink-03)", padding: "1rem", borderRadius: 10 }}>
              <h3 style={{ fontSize: "0.85rem", fontWeight: 600, display: "flex", alignItems: "center", gap: "0.4rem", margin: 0 }}>
                <UserCheck size={14} className="text-iris" /> Target Candidate Context
              </h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                  <span style={{ fontSize: "0.72rem" }}>Job Listing</span>
                  <select className="input" value={selectedJobId} onChange={e => setSelectedJobId(e.target.value)} style={{ padding: "0.35rem", fontSize: "0.82rem", background: "var(--surface)" }}>
                    <option value="">-- No specific Job --</option>
                    {jobs.map(j => (<option key={j.id} value={j.id}>{j.title} {j.team ? `(${j.team})` : ""}</option>))}
                  </select>
                </label>
                <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                  <span style={{ fontSize: "0.72rem" }}>Candidate</span>
                  <select className="input" value={selectedCandidateUserId} onChange={e => setSelectedCandidateUserId(e.target.value)} style={{ padding: "0.35rem", fontSize: "0.82rem", background: "var(--surface)" }}>
                    <option value="">-- No specific Candidate --</option>
                    {candidates.map(c => (<option key={c.candidate_user_id} value={c.candidate_user_id}>{c.full_name || c.email} ({c.job_title})</option>))}
                  </select>
                </label>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                {selectedTemplate.placeholders.length > 0 && (
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      >
                        {isPopulating ? (<><Loader2 size={12} className="animate-spin" /> Resolving...</>) : (<>✦ Auto-fill via AI context</>)}
                      </button>
                    </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem", background: "var(--ink-03)", padding: "1rem", borderRadius: 10 }}>
                  ...
                  <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem" }}>
                    <span style={{ fontSize: "0.72rem" }}>Special instructions (optional)</span>
                    <textarea
                      className="input" rows={2} value={customPrompt}
                      placeholder="e.g. Schedule for Friday at 10 AM, keep it friendly..."
                  ...
                  >
                    {isGenerating ? (<><Loader2 size={16} className="animate-spin" /> Generating...</>) : (<><Sparkles size={16} /> Draft with Aura AI</>)}
                  </button>
                </div>
              </div>
